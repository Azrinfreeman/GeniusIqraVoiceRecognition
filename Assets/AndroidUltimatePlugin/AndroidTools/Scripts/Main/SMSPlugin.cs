...
namespace Gigadrillgames.AUP.Tools
{
    public class SMSPlugin : MonoBehaviour
    {
        ...
        private void SetCallbackListener(Action onSMSComplete, Action onSMSFail,
            Action<string, string, string> onReceivedSMS)
        {
#if UNITY_ANDROID
            if (Application.platform == RuntimePlatform.Android)
            {
                ...
                Utils.Message(TAG, "setCallbackListener");
            }
            else
            {
                Utils.Message(TAG, "warning: must run in actual android device");
